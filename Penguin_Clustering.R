...
# riflessione finale su chi è meglio tra PCA e non PCA

subplot(KMEANS1_S, KMEANS2_S)
...
# da un approccio deterministico a un approccio probabilistico al clustering, attraverso i mixture models
...
# Il vantaggio chiave dell'approccio basato su modelli, rispetto ai metodi di clustering standard (k-means, k-medoids, diana, ecc.), è la possibilità di suggerire il numero di cluster e un modello appropriato.
# Invece di adottare un approccio euristico per costruire un cluster, il clustering basato su modello utilizza un approccio basato sulla probabilità. Questo approccio presuppone che i dati siano generati da una distribuzione di probabilità sottostante e cerca di recuperare tale distribuzione dai dati stessi.
# Un comune approccio basato su modello è quello che utilizza i finite mixture models, che forniscono un quadro di modellazione flessibile per l'analisi della distribuzione di probabilità.
# Tali modelli sono una **somma linearmente ponderata di distribuzioni di probabilità dei componenti**.

# Nel clustering basato su modello, i dati  considerati sono provenienti da una distribuzione che è una miscela (mixture) di due o più componenti (cioè i cluster).
# Ogni componente k (cioè gruppo o cluster) è modellata dalla distribuzione normale o gaussiana,
# caratterizzata da parametri quali: vettore media, matrice di covarianza, probabilità associata
# (ogni punto ha una probabilità di appartenere a ciascun cluster).

# Ciascun cluster k è centrato sulla media, con una maggiore densità per i punti vicini alla media.
# Le caratteristiche geometriche (forma, volume, orientamento) di ciascun cluster sono determinate dalla matrice di covarianza.

# Similmente a quanto avvenuto prima di implementare gli algoritmi di clustering deterministico, quando si è ricercato l'appropriato valore di k, anche
# nel caso dei mixture models ha senso fare un'analisi preliminare utilizzando il "Likelihood Ratio Test", per identificare il numero di componenti della miscela:

## Bootstrap sequential LRT (Likelihood Ratio Test) 

LRT <- mclustBootstrapLRT(df_mm, modelName = "VEE") #varying volume, equal shape, equal orientation (ellipsoidal covariance), approfondito più avanti
LRT

# da in output due valori: la statistica LRT che più è grande e più indica che il modello con più componenti è da preferire
# e il p-value del bootstrap.
# p-value <0.05 si rifiuta l'ipotesi nulla che il modello migliore abbia solo una componente (statisticamente è più probabile che un modello con un maggior numero di componenti sia un miglior fit per il dataset)
# p-value > 0.05 NON si rifiuta l'ipotesi nulla e sia ssume che il modello corrente, con un numero di componenti inferiore sia sufficiente per descrivere il dataset in esame.

# giunti a valutare un modello con 4 componenti, contro un modello con 5, non si rifiuta finalmente l'ipotesi nulla e si assume che 5 componenti siano sufficienti perché il modello descriva in mnaiera apporpriata il dataset.

# si puo' di conseguenza procedere all'applicazione del mixture model con questo dato noto. È già stato identificato il plot di densità generato dal GMM: Gaussian Mixture model, presentato qualche sezione fa:
...
# questo modello utilizza l'algoritmo EM (Expectation-Maximization): un metodo iterativo utilizzato per calcolare la probabilità che un punto dato appartenga a ciascun cluster,data la sua posizione e i parametri attuali del modello.

summary(multiv_model, parameters=TRUE)

#Il summary fornisce alcune informazioni sui gradi di libertà, verosomiglianza logaritmica, sugli indici BIC e ICL del modello.

# La verosomiglianza logaritmica, una misura del fit del modello, è pari a circa -1.100. Un valore molto negativo di questo indice indica che il modello non si adatta perfettamente ai dati.
# Ad ogni modo, l'interpretazione di questo valore preso da solo non è significativa. Combinandolo con le altre informazioni derivate dal summary si giunge ad una conclusione diversa:
# I valori di BIC (Bayesian Information Criterion) e ICL (Integrated Completed Likelihood) sono entrambi intorno al -2.450, con valori molto negativi che indicano un buon fit.
# Infine, il modello ha 32 gradi di libertà, cosa che indica una complessità moderata, e la table con il numero di osservazioni per cluster rivela 4 clusters con 149, 65, 61, 58 esemplari rispettivamente.
# Complessivamente, Considerando tutte le metriche, il modello suggerisce un buon adattamento ai dati con cluster ben definiti.

# tuttavia si noti che la distribuzione delle osservazioni per cluster non è perfettamente bilanciata.

# Per giungere a questi risultati, la procedura che è stata seguita è quella di eseguire una ricerca su un insieme di diversi tipi di funzioni gaussiane per trovare il modello migliore tra tutti in base al criterio BIC.

plot(multiv_model, what = "BIC")

# se al modello non fosse stato dato in input nessun valore di G, il grafico avrebbe un aspetto diverso, mostrerebbe delle curve e non più dei punti, che rappresentano l'evoluzione del fit del modello per valori crescenti di G: 

plot((Mclust(df_mm)), what = "BIC") # prenderà di default G=1:9

# anche qui in ogni caso, si giuge alla stessa conclusione, poiché in corrispondenza di G=4 ad avere valore maggiore è il modello VEE.

# Tale acronimo, così come i restanti nella legenda, sono combinazioni delle lettere E, V e occasionalmente I.
# E sta per Equal, V sta per Varying. Fanno riferimento, nell'ordine in cui sono scritte, rispettivametne al volume, la forma e l'orientamento dei cluster.
# Per esempio:
...
# VEE: varying volume, equal shape, equal orientation (ellipsoidal covariance)

# Come già detto è l'ultimo (VEE) ad adattarsi meglio al dataset.
...
# da questo summary si vedono le 4 direzioni in cui è stato
...
anomalies <- GMM_df[anomaly_scores > threshold, ]

# Visualize the results
plot(GMM_df, pch = 19, col = ifelse(anomaly_scores > threshold, "#FF7F50", "#99D9EA"),
...
pairs(wine[2:14], col=factor(wine$Type), pch=20, cex=0.5)
...
plot(wine.lda.values$x[,1],wine.lda.values$x[,2],cex=0.8, col=Type, pch=20, xlab="LDA axis 1", ylab="LDA axis 2") # make a scatterplot
...
# the first and second discriminant functions together, since the first discriminant function 
# can separate cultivars 1 and 3 very well, and the second discriminant function can separate 
# cultivars 1 and 2, and cultivars 2 and 3, reasonably well. 
wine.lda.values$class
confMat.LDA<-confusionMatrix(Type,wine.lda.values$class)$table
confMat.LDA
accuracy.LDA<-sum(diag(confMat.LDA))/(nrow(wine))*100
accuracy.LDA
